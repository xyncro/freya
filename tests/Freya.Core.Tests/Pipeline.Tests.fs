...
let ``pipeline executes both monads if first returns next`` () =
    let o1 = Freya.State.map (fun x -> x.Environment.["o1"] <- true; x) *> Freya.Pipeline.next
    let o2 = Freya.State.map (fun x -> x.Environment.["o2"] <- true; x) *> Freya.Pipeline.next
...
let ``pipeline executes only the first monad if first returns terminate`` () =
    let o1 = Freya.State.map (fun x -> x.Environment.["o1"] <- true; x) *> Freya.Pipeline.halt
    let o2 = Freya.State.map (fun x -> x.Environment.["o2"] <- true; x) *> Freya.Pipeline.next
