...
type ProtectedStream(innerStream : Stream) = 
    ...
    do 
        if innerStream = null then raise (ArgumentNullException("innerStream"))
    ...
    override x.CanRead = 
        ...
        else innerStream.CanRead
    
    override x.CanSeek = 
        if disposed then false
        else innerStream.CanSeek
    
    override x.CanTimeout = innerStream.CanTimeout
    
    override x.CanWrite = 
        if disposed then false
        else innerStream.CanWrite
    ...
    override x.Position 
        ...
        and set (v) = 
            raiseIfDisposed()
            innerStream.Position <- v
    
    override x.ReadTimeout 
        with get () = 
            raiseIfDisposed()
            innerStream.ReadTimeout
        and set (v) = 
            raiseIfDisposed()
            innerStream.ReadTimeout <- v
    
    override x.WriteTimeout 
        with get () = 
            raiseIfDisposed()
            innerStream.WriteTimeout
        and set (v) = 
            raiseIfDisposed()
            innerStream.WriteTimeout <- v
    ...
    override x.EndRead(asyncResult) = 
        raiseIfDisposed()
        innerStream.EndRead(asyncResult)
    
    override x.EndWrite(asyncResult) = 
        raiseIfDisposed()
    ...
    let fromAsyncSystemNetHttp (handler : HttpRequestMessage -> Async<HttpResponseMessage>) = 
        OwinAppFunc(fun env -> 
            let request = 
                ...
                |> Option.get
            async { let! response = handler request
